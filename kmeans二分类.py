...
class KMeans():
    ...
    def fit(self, data_map, max_epoch=20):
        ...
        cluster_info = {}
        for id in data_map:
            cluster_info[id] = {"feature": data_map[id],
                                "cluster_label": self.find_nearest_cluster_center(data_map[id], init_cluster_centers)}
        ...
        for epoch in range(max_epoch):
            ...
            for id in data_map:
                cluster_info[id] = {"feature": data_map[id],
                                    "cluster_label": self.find_nearest_cluster_center(data_map[id],
                                                                                      new_cluster_centers)}
    ...
    def generate_init_cluster_centers(self, data_map):
        ...
        return cluster_centers#输出[array([178.,  75.]), array([10. ,  0.1])]
    ...
    def distance(self, x1, x2, type="eu"):
        d = None
        if type == "eu":
            d = np.sum((x1 - x2) ** 2)  # 欧氏距离d=|x1-x2|2+|y1-y2|2 开根号
        return d
...
老鼠舒克    10    0.1"""
        ...
        for line in data_list[1:]:
            data = line.split("    ")
            #list(map(lambda x: x ** 2, [1, 2, 3, 4, 5]))表示返回数字的平方的list,lambda类似函数作用
